...
include { visualization; get_metadata; clustering; heatmap; volcano_plot; enrichment; gsea } from "$params.nf_script/analysis.nf"
...
workflow{
    ...
    //heatmap(clustered_h5ad, channel.value('clustering'), channel.value('3')) // = nb genes
    //volcano_plot(clustered_h5ad, channel.value('clustering'), channel.value(['0', '4']), '10')  
    //enrichment(clustered_h5ad, channel.value('clustering'), channel.value(['0', '4']), '10') | view //put 'rest' on the second of .value to compare to all the others

    //Alteration
    //altered_h5ad1 = remove_cells(clustered_h5ad, channel.value('clustering'), channel.value(['7', '12', '13']))
